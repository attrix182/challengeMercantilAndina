...
export class FormDatosVehiculoComponent extends BaseFormAbstract implements OnInit {
    ...
    onMarcaAnioChange() {
        ...
        this.getModelosByMarcaAnio(this.formGroup.value.marca, this.formGroup.value.anio);
    }

    onVersionChange() {
        this.getVersiones(this.formGroup.value.marca, this.formGroup.value.modelo, this.formGroup.value.anio);
    }

    nextStep() {
        const vehiculo = this.formGroup.value;
        vehiculo.marcaCompleta = this.marcas.find((marca) => marca.id === this.formGroup.value.marca);
        vehiculo.versionCompleta = this.versiones.find((version) => version.id === this.formGroup.value.version);
