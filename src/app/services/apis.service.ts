...
export class ApisService {
  ...
  getCiudades(idProvincia: number): Observable<any> {
    return this.http.get(
      `https://apis.datos.gob.ar/georef/api/municipios?provincia=${idProvincia}`
    );
  }

  verifyUser(nombre: string): Observable<any> {
    return this.http.get(
      `https://servicios.qamercantilandina.com.ar/api_mock_frontend/v1/usuarios?nombre=${nombre}`
    );
  }
  ...
  getModelos(idMarca: any, anio: any): Observable<any> {
    return this.http.get(
      `https://servicios.qamercantilandina.com.ar/api/v1/vehiculos/marcas/${idMarca}/${anio}`
    );
  }

  getVersiones(idMarca: any, anio: any, idModelo: any): Observable<any> {
    return this.http.get(
      `https://servicios.qamercantilandina.com.ar/api/v1/vehiculos/marcas/${idMarca}/${anio}/${idModelo}`
    );
  }
